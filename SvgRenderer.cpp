...
// for original code, use https://github.com/ad-freiburg/loom
// below code is slightly modified for the TenneT project
// no bold printing of labels
// use font London Tube
// make certain line colors wider
// don't print stations that start with a digit
  ...
  for (auto n : outG.getNds()) {
    ...
    if (_cfg->renderStations && n->pl().stops().size() > 0 &&
      ...
      for (const auto& geom : outG.getStopGeoms(n, _cfg->tightStations, 32)) {
          ...
          if (stationName.size() >= 2 && std::isdigit(stationName[2])) {
  ...
  for (const auto& c : renderCliques) {
    ...
    for (size_t i = 0; i < c.geoms.size(); i++) {
      ...
      styleStr << ";stroke-linecap:round;stroke-opacity:1;stroke-width:"
              //  << _cfg->lineWidth * _cfg->outputResolution; //removed
               << strokeWidth; //changed
      Params params;
...
void SvgRenderer::renderLinePart(const PolyLine<double> p, double width,
  ...
  styleOutline << "fill:none;stroke:#000000;stroke-linecap:round;stroke-width:"
               << (width + _cfg->outlineWidth) * 0.8 * _cfg->outputResolution << ";"
               << oCss;
